...
def generate(command):
    ...
    if export_video:
        ...
        if export_texmap:
            ...
            return output_video, [output_model_obj, mesh_basename+'.mtl', mesh_basename+'.png']
        else:
            return output_video, [output_model_obj]
    else:
        output_model_obj = make3d(mv_images, model, device, IS_FLEXICUBES, infer_config, export_video, export_texmap)
        if export_texmap:
            mesh_basename = os.path.splitext(output_model_obj)[0]
            return [output_model_obj, mesh_basename+'.mtl', mesh_basename+'.png']
        else:
...
with gr.Blocks(css=".gradio-container {max-width: 544px !important}", analytics_enabled=False) as demo:
    ...
    with gr.Row():
        output_files = gr.File(show_label=False)
    button.click(fn=generate, inputs=[textbox], outputs=[output_video, output_files], show_progress=True)
